...
export const UploadCard: React.FC<UploadCardProps> = ({ upload, onDelete, onRenderReady }) => {
  ...
  const handlePublishToSite = async () => {
    ...
    if (!upload.serviceSlug || !upload.teamSlug) {
      setSitePublishNote('РќРµ РІС‹Р±СЂР°РЅС‹ РґРёСЃС†РёРїР»РёРЅР°/Р±СЂРёРіР°РґР° РґР»СЏ РѕС‚РїСЂР°РІРєРё.');
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        setSitePublishNote('РћС‚РїСЂР°РІР»РµРЅРѕ РІ webhook СЃР°Р№С‚Р°.');
      } else {
        setSitePublishNote(
          `РћС€РёР±РєР° РѕС‚РїСЂР°РІРєРё${result.statusCode ? ` (${result.statusCode})` : ''}: ${result.message || 'unknown'}`
        );
      }
    } catch (err) {
      setSitePublishNote(
        `РћС€РёР±РєР° РѕС‚РїСЂР°РІРєРё: ${err instanceof Error ? err.message : 'unknown'}`
      );
    } finally {
  ...
  const timeAgo = (dateStr: string): string => {
    ...
    return `${Math.floor(hours / 24)} РґРЅ. РЅР°Р·Р°Рґ`;
  };

  // Summary line: "3 С„РѕС‚Рѕ В· 2 РІРёРґРµРѕ В· С‚РµРєСЃС‚"
  const summaryParts: string[] = [];
  const photoCount = upload.uploadedPhotos?.length ?? 0;
  const videoCount = upload.uploadedVideos?.length ?? 0;
  if (photoCount > 0) summaryParts.push(`${photoCount} С„РѕС‚Рѕ`);
  if (videoCount > 0) summaryParts.push(`${videoCount} РІРёРґРµРѕ`);
  if (upload.textContent) summaryParts.push('С‚РµРєСЃС‚');
  // fallback for legacy single-file uploads
  if (summaryParts.length === 0) {
    const labels: Record<string, string> = { photo: 'Р¤РѕС‚Рѕ', video: 'Р’РёРґРµРѕ', text: 'РўРµРєСЃС‚', voice: 'Р“РѕР»РѕСЃ', mixed: 'РњР°С‚РµСЂРёР°Р»С‹' };
  ...
  return (
    <div className="card border border-gray-100 hover:shadow-md transition-shadow">
      ...
      {/* в”Ђв”Ђ Header row в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ */}
      ...
      {/* в”Ђв”Ђ Photo thumbnails в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ */}
      {photoCount > 0 && (
      ...
      {/* в”Ђв”Ђ Video thumbnails (placeholder icons for uploaded source videos) в”Ђв”Ђ */}
      {videoCount > 0 && (
      ...
      {/* в”Ђв”Ђ Legacy single-file thumbnail (old records) в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ */}
      ...
      {/* в”Ђв”Ђ Processing placeholder (waiting for n8n URL) в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ */}
      {upload.status === 'processing' && upload.triggeredN8n && !upload.renderUrl && (
        <div className="mt-4 border-t border-gray-100 pt-4">
          <div className="rounded-xl bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-100 p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-semibold text-blue-800">РћС‚РїСЂР°РІР»РµРЅРѕ РЅР° РѕР±СЂР°Р±РѕС‚РєСѓ</p>
              <p className="text-xs text-blue-500 mt-0.5">n8n РѕР±СЂР°Р±Р°С‚С‹РІР°РµС‚ РґР°РЅРЅС‹Рµ, РѕР¶РёРґР°РµРј РІРёРґРµРѕ...</p>
            </div>
          </div>
        </div>
      )}

      {/* в”Ђв”Ђ Render video block в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ */}
      {hasRender && (
        <div className="mt-4 border-t border-gray-100 pt-4">
          {videoReady ? (
            <>
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">
                Р“РѕС‚РѕРІРѕРµ РІРёРґРµРѕ
              </p>
              ...
              <div className="flex flex-wrap gap-3 mt-3">
                <button
                  ...
                  className="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-primary-600 text-white text-sm font-semibold rounded-lg hover:bg-primary-700 transition-colors"
                >
                  <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" />
                  </svg>
                  {publishOpen ? 'РЎРєСЂС‹С‚СЊ' : 'РћРїСѓР±Р»РёРєРѕРІР°С‚СЊ'}
                </button>
                <button
                ...
                <button
                  disabled
                  className="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-gray-100 text-gray-400 text-sm font-semibold rounded-lg cursor-not-allowed"
                  title="Р‘СѓРґРµС‚ РґРѕСЃС‚СѓРїРЅРѕ РІ СЃР»РµРґСѓСЋС‰РµР№ РІРµСЂСЃРёРё"
                >
                  <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                  </svg>
                  РќРѕРІР°СЏ РёС‚РµСЂР°С†РёСЏ
                </button>
              </div>
              {sitePublishNote && (
                <p className="mt-2 text-xs text-gray-600">{sitePublishNote}</p>
              )}
            </>
          ) : (
            <div className="rounded-xl bg-gradient-to-br from-purple-50 to-blue-50 border border-purple-100 p-5 flex flex-col items-center gap-3 text-center">
              ...
              <div>
                <p className="text-sm font-semibold text-purple-800">Р’РёРґРµРѕ РіРѕС‚РѕРІРёС‚СЃСЏ...</p>
                <p className="text-xs text-purple-500 mt-1">Creatomate СЂРµРЅРґРµСЂРёС‚ С„Р°Р№Р». РџСЂРѕРІРµСЂСЏРµРј РєР°Р¶РґСѓСЋ РјРёРЅСѓС‚Сѓ.</p>
              </div>
      ...
      {/* в”Ђв”Ђ Publish panel в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ */}
      {publishOpen && hasRender && (
        <div className="mt-4 border-t border-gray-100 pt-4">
          <p className="text-sm font-semibold text-gray-700 mb-3">РџРѕРґРµР»РёС‚СЊСЃСЏ:</p>
          <div className="flex flex-wrap gap-2">
            ...
            <button onClick={handleCopyLink}
              ...
              {copied ? (
                <><svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>РЎРєРѕРїРёСЂРѕРІР°РЅРѕ!</>
              ) : (
                <><svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>РљРѕРїРёСЂРѕРІР°С‚СЊ СЃСЃС‹Р»РєСѓ</>
              )}
            </button>
